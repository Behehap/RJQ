...
# مستندات فنی پروژه صف کار مقاوم
...
- **لایه دریافت درخواست:** دریافت درخواست‌های HTTP و تبدیل آن‌ها به ساختار کار.
...
۲. یک نمونه کار با شناسه یکتای UUID ساخته می‌شود و وضعیت آن «در انتظار» قرار می‌گیرد.
...
- **شکست:** شمارنده تلاش افزایش می‌یابد. اگر کمتر از حد مجاز باشد، کار دوباره به صف برمی‌گردد. در غیر این صورت «شکست‌خورده» می‌شود.
- **پیش‌دستی:** اگر کار فوری وارد شود و همه کارگرها مشغول باشند، یک کار معمولی لغو شده و بدون جریمه به صف بازمی‌گردد.
...
اگر یک کار فوری وارد شود و همه کارگرها مشغول پردازش کارهای معمولی باشند، سیستم یک کار معمولی را متوقف می‌کند تا کار فوری همان لحظه پردازش شود.
...
- متد `Enqueue` از نسخه امن صف اصلی استفاده می‌کند.
...
- دکمه تلاش مجدد در کنار کارهای شکست‌خورده قرار دارد.
...
راه‌حل: حذف بازنویسی `Enqueue` و استفاده از نسخه امن صف اصلی.
